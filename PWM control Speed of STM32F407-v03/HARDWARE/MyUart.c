...
void USART1_IRQHandler(void)
{
	...
	if(USART_GetITStatus(USART1,USART_IT_RXNE)==SET)	//判断是不是接受中断
	{
		...
		{
			case 'A':
				Style_pai =1;break;//命令：排风扇关
			case 'B':
				Style_pai =2;break;//命令：排风扇一档
			case 'C':
				Style_pai =3;break;//命令：排风扇二档
			case 'D':
				Style_pai =4;break;//命令：排风扇三档
			
			
			case 'a':
				Style_san =1;break;//命令：散热器关
			case 'b':
				Style_san =2;break;//命令：散热器一档
			case 'c':
				Style_san =3;break;//命令：散热器二档
			case 'd':
				Style_san =4;break;//命令：散热器三档
		}
	}						
}
